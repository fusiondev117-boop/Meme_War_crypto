...
const AuthenticationModal = ({ open, setOpen, authType }) => {
    ...
    useEffect(() => {
        if (active && account) {
            const userMetamaskLogin = async () => {
                try {
                    console.log('Attempting MetaMask login with account:', account);
                    ...
                    if (response && response.status) {
                        ...
                        const balanceData = await getMyBalances({ userId: response.userData._id });
                        if (balanceData.status) {
                            dispatch({ type: 'SET_BALANCEDATA', data: balanceData.data });
                        }
                        else {
                            addToast(balanceData.message || 'Failed to load balances', { appearance: 'error', autoDismiss: true });
                        }
                    }
                    else {
                        const errorMsg = response?.message || 'MetaMask login failed';
                        console.error('MetaMask login failed:', errorMsg);
    ...
    const googleLoginSuccess = async (success) => {
        try {
            ...
            addToast(error.message || 'Failed to login with Google', { appearance: 'error', autoDismiss: true });
        }
    }
