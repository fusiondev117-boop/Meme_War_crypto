...
const MainHeader = () => {
    ...
    const currency = authData.isAuth && authData.userData.currency ? authData.userData.currency : { coinType: 'BTC', type: '' };
    ...
    return (
        <header className={classes.MainHeaderBox}>
                        ...
                        >
                            {
                                currencies && currencies.length > 0 && currencies.map((currency, index) => {
                                    if (!currency || !currency.name) return null;
                                    
                                    const defaultBalance = { balance: 0, type: currency.token || '', coinType: currency.name };
                                    let currencyBalance = defaultBalance;
                                    
                                    if (authData.balanceData && authData.balanceData.length > 0) {
                                        const found = authData.balanceData.find((data) => data.coinType === currency.name && data.type === currency.token);
                                        if (found) currencyBalance = found;
                                    }
                                    ...
                                    return (
                                        <MenuItem key={index} value={JSON.stringify({ coinType: currency.name, type: currencyBalance.type || currency.token || '' })} className={classes.CustomMenuItem}>
                                            ...
                                            <span className={classes.CoinAmountSpan}>
                                                {
                                                    (currencyBalance.balance || 0).toFixed(currency.decimal || 2)
                                                }
                                            </span>
            ...
            <Box className={classes.LoginBox}>
                ...
                <Button 
                    className={clsx(classes.HeaderButton, classes.CustomizeButton)}
